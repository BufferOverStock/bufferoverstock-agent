...
from tools.security import generate_security_report
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Analyze raw vulnerability scan logs (like Nmap, WPScan, Nessus, Nikto) using LLM and generate a professional, formatted Markdown security report.",
            "parameters": {
                "type": "object",
                "properties": {
                    "scan_file_path": {"type": "string", "description": "Absolute or relative path to the raw security scan log file."},
                    "output_report_path": {"type": "string", "description": "Absolute or relative path where the generated Markdown security report should be saved."}
                },
                "required": ["scan_file_path", "output_report_path"]
            }
        }
    }
]
...
1. If the user asks you to analyze a security scan or generate a security report, call the generate_security_report tool immediately with the specified file paths.
2. For general questions, ALWAYS check query_knowledge first before anything else.
3. If the knowledge base has a good answer, use it directly.
4. If not, use web_search to find the answer.
5. After a successful web_search, you MUST call save_to_knowledge with:
   - topic: a short title for what was learned (5 words max)
   - summary: a clean 2-3 sentence summary of the key facts
6. Then give the user a clear, well-explained final answer.

Never skip step 5 after a web search. Learning from every query makes you smarter."""
...
def dispatch(tool_name: str, args: dict) -> str:
    ...
    try:
        if tool_name == "web_search":
            ...
            return generate_security_report(args["scan_file_path"], args["output_report_path"])
        else:
...
if __name__ == "__main__":
    print("=== BufferOverStock Security Agent ===")
